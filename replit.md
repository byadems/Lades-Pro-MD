...
- **Reconnect policy (HOTFIX)**: `core/bot.js` connection-close handler now classifies `DisconnectReason` codes (Hermit-bot + KnightBot-Mini referansı). **Permanent** (loggedOut 401 / forbidden 403 / connectionReplaced 440 / multideviceMismatch 411) → no retry. `connectionReplaced` & `forbidden` → suspend session, notify dashboard, do NOT reconnect (prevents infinite ping-pong with another active device). **Fast-retry transient** (restartRequired 515 / connectionClosed 428 / connectionLost 408 / unavailableService 503) → fixed 3s delay, retry-count capped at 1 (these are normal handshake events). Stale WebSocket explicitly closed before re-spawn.
