...
async function nexGet(path, opts = {}) {
  try {
    ...
    if (e.name === "CircuitBreakerError") {
      throw new Error("⚠️ *API servisi şu an yoğun veya kapalı. Lütfen daha sonra tekrar deneyin!*");
    }
    throw e;
  }
}
...
Module({
  ...
  async (message, match) => {
    ...
    if (emojis.length < 2) {
      return await message.sendReply("⚠️ *İki farklı emoji girin!* \n\n💬 *Kullanım:* \`.emojimix 😎 🔥\`");
    }
    try {
      ...
      await message.sendReply(`❌ *Emoji birleştirilemedi:* \n\n${emsg}`);
    }
  }
);
...
Module({
  ...
  async (message, match) => {
    ...
    if (!text) return await message.sendReply("⚠️ *Metin girin!* \n\n💬 *Kullanım:* \`.yazı LADES\`");
    try {
...
Module({
  ...
  async (message, match) => {
    ...
    if (words.length < 2) return await message.sendReply("⚠️ *İki kelime girin!* \n\n💬 *Kullanım:* \`.marvel LADES BOT\`");
    try {
...
Module({
  ...
  async (message, match) => {
    ...
    if (!query) return await message.sendReply("⚠️ *Şarkı adı girin!* \n\n💬 *Kullanım:* \`.şarkısözü Tarkan Şımarık\`");
    ...
    if (!lyrics) {
      return await message.sendReply("❌ *Şarkı sözü bulunamadı!* _Lütfen `Şarkıcı - Şarkı adı` formatında tekrar deneyin._");
    }
...
Module({
  ...
  async (message, match) => {
    const query = (match[1] || "").trim();
    if (!query) return await message.sendReply("⚠️ *Konu girin!* \n\n💬 *Kullanım:* \`.duvarkağıdı doğa\`");
    ...
    if (!imgUrl) {
      return await message.sendReply(`❌ *Duvar kağıdı bulunamadı:* \n\n${lastError}`);
    }
    ...
    try {
      ...
      await message.sendReply(`❌ *Görsel gönderilemedi:* \n\n${sendErr.message}`);
    }
  }
);
...
Module({
  ...
  async (message, match) => {
    const query = (match[1] || "").trim();
    if (!query) return await message.sendReply("⚠️ *Çıkartma konusu girin!* \n\n💬 *Kullanım:* \`.çıkartmabul yavru köpek\`");
    try {
      ...
      await message.sendReply(`❌ *Çıkartma bulunamadı:* \n\n${e.message}`);
    }
  }
);
...
Module({
  ...
  async (message, match) => {
    ...
    try {
      ...
      await message.sendReply(`❌ *Vikipedi araması başarısız:* \n\n${e.message}`);
    }
  }
);
...
Module({
  ...
  async (message, match) => {
    ...
    if (!text) return await message.sendReply("⚠️ *Metin girin veya bir mesajı yanıtlayın!* \n\n💬 *Kullanım:* \`.alıntı Merhaba!\` ");
    ...
    try {
      ...
      await message.sendReply(`❌ *Alıntı oluşturulamadı:* \n\n${e.message}`);
    }
  }
);
...
Module({
  ...
  async (message, match) => {
    ...
    if (!prompt) return await message.sendReply("⚠️ *Görsel açıklaması girin!* \n\n💬 *Kullanım:* \`.görsel sevimli bir köpek\`");
    try {
      ...
      await message.edit("✅ *Görsel oluşturuldu!*", message.jid, sent.key);
      await message.client.sendMessage(message.jid, {
        image: buf,
        caption: `🎨`,
      }, { quoted: message.data });
...
Module({
  ...
  async (message, match) => {
    ...
    if (!url || !/threads\.net/i.test(url)) {
      return await message.sendReply("⚠️ *Threads bağlantısı girin!* \n\n💬 *Kullanım:* \`.threads <bağlantı>\` ");
    }
    ...
    try {
      ...
      if (!mediaUrls || !mediaUrls.length) {
        return await message.sendReply("⚠️ *Medya bulunamadı veya bağlantı geçersiz!*");
      }
      ...
      await message.sendReply(`ℹ️ _${mediaUrls.length} medya iletiliyor..._`, { quoted: quotedMessage });
      ...
      await message.sendReply(`❌ *Threads indirme başarısız:* \n\n${e.message}`);
    }
  }
);
...
Module({
  ...
  async (message, match) => {
    ...
    if (!url || !url.includes("soundcloud")) {
      return await message.sendReply("⚠️ *SoundCloud bağlantısı girin!* \n\n💬 *Kullanım:* \`.soundcloud <bağlantı>\` ");
    }
    let statusMsg;
    try {
      statusMsg = await message.sendReply("🔻 _SoundCloud'dan indiriliyor..._");
      ...
      await message.edit("✅ *İndirme tamamlandı!*", message.jid, statusMsg.key);
    } catch (e) {
      ...
      if (statusMsg) {
        await message.edit(`❌ *SoundCloud indirme başarısız:* \n\n${e.message}`, message.jid, statusMsg.key);
      } else {
        await message.sendReply(`❌ *SoundCloud indirme başarısız:* \n\n${e.message}`);
      }
    }
  }
);
...
Module({
  ...
  async (message, match) => {
    try {
      ...
      if (parts.length < 2) {
        return await message.sendReply("⚠️ *Kullanımı:*\n_(Bir mesaja yanıtlayarak) .çevir en tr_");
      }
      const src = parts[0];
      const dst = parts[1];
...
Module({
  ...
  async (message, match) => {
    const input = (match[1] || "").trim();
    const parts = input.split(/\s+/);
    if (parts.length < 2) return await message.sendReply("⚠️ *En az iki kelime girin!* \n\n💬 *Kullanım:* \`.mascot LADES wolf\`");
    const text1 = parts[0];
